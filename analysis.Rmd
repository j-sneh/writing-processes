...
```
It is clear that our KMeans algorithm does a good job clustering, but when considering the underlying relation to scores it fails to achieve seperability between clusters. In other words, while the clusters within seem strong and cohesive, they tend to be overlapping with other clusters (i.e. scores are overlapping amongs clusters) making k-means an overall bad fit for this kind of non-seperable data. 
...
acc_data
```
It seems like most of our predictions are going into cluster 1. This seems like a bad algorithm too, there's lots of scores overlapping into 1 cluster! In general, since the data is not seperable and not globular in structure it is hard for clustering algorithms to perform well. This requires more time and work to mitigate these issues. Some techniques include scaling and centering our data, creating other features, maybe even performing a PCA to project our data onto a different to extract the principal dimension. Due to the nature of our short-windowed project, we would like to move onto Supervised Learning Algorithms, but keep in mind that these insights will be relevant! 
...
prop.k1 <- kable(prop.t1, caption = "Propodds Training Confusion Matrix") %>% kable_styling(full_width = F, position = "left", latex_options = c("scale_down", "HOLD_position"))
prop.k2 <- kable(prop.t2, caption = "Propodds Testing Confusion Matrix") %>% kable_styling(full_width = F, position = "right", latex_options = c("scale_down", "HOLD_position"))
...
# Confusion Matrix
(table(knn.train.pred, train.supervised$score))
# Accuracy
...
# Confusion Matrix
(table(knn.test.pred, test.supervised$score))
# Accuracy
...
# confusion matrix
(table(train.labeled_predictions, train.supervised$score))
# train accuracy
...
# confusion matrix
(table(test.labeled_predictions, test.supervised$score))
